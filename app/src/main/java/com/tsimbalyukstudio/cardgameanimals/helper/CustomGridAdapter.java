...
import com.tsimbalyukstudio.cardgameanimals.CUSTOMIMG;
import com.tsimbalyukstudio.cardgameanimals.R;
import com.tsimbalyukstudio.cardgameanimals.logic.GameLogic;
import com.tsimbalyukstudio.cardgameanimals.ui.Activities.CustomActivity;
...
public class CustomGridAdapter extends BaseAdapter implements Animation.AnimationListener {
    ...
    private int randomInt = R.drawable.covercustom;

    private Animation animation1;

    ImageView iv;

    public CustomGridAdapter(Context c, int countCards, int countColumn) {
        mContext = c;
    ...
    public View getView(int position, final View convertView, final ViewGroup parent) {
        ...
        if (convertView == null) {
                    ...
                    if (gl.compare() && isFirstClick) {
                        ...
                        handler.postDelayed(new Runnable() {
                            ...
                            public void run() {
                                ...
                                iv.setBackgroundResource(R.drawable.covercustom);
                                try {
                                    chosedView.get(0).setBackgroundResource(R.drawable.covercustom);
                                } catch (Exception e) {
                                }
                                chosedView.clear();
            ...
            imageView.setBackgroundResource(R.drawable.covercustom);
    ...
    public void createGrid() {
        ...
        for (int x = 0; x < (countCards / 2); x++) {
            ...
            int resId = CUSTOMIMG.values()[(int) (Math.random() * CUSTOMIMG.values().length)].resId;

            if (x != 0) {
                for (int i = 0; i < tempRes.length; i++) {
                    if (tempRes[i] == resId) {
                        resId = CUSTOMIMG.values()[(int) (Math.random() * CUSTOMIMG.values().length)].resId;
                        i = 0;
                    }
                }
            }
